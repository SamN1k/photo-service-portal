...
export const offerService = {
    async listOffers(params: OfferListParams = {}): Promise<PaginatedResult<PhotoOffer>> {
        const { data } = await apiClient.get<PaginatedResult<PhotoOffer>>('/offers', { params });
        return data;
    },

    async getOffer(offerId: string): Promise<PhotoOffer> {
        const { data } = await apiClient.get<PhotoOffer>(`/offers/${offerId}`);
        return data;
    },

    async createOffer(input: OfferInput, photographerId: string, photographerName: string): Promise<PhotoOffer> {
        const { data } = await apiClient.post<PhotoOffer>('/offers', {
            ...input,
            photographerId,
            photographerName,
        });
    ...
    async updateOffer(offerId: string, input: OfferInput): Promise<PhotoOffer> {
        const { data } = await apiClient.put<PhotoOffer>(`/offers/${offerId}`, input);
        return data;
    },
