...
const Offers = () => {
    ...
    const [category, setCategory] =
        useState<'all' | OfferCategory>('all');

    const [sortBy, setSortBy] =
        useState<OfferSort>('newest');

    const [page, setPage] = useState(1);

    const [result, setResult] =
        useState<PaginatedResult<PhotoOffer> | null>(null);
    ...
    const loadOffers = useCallback(
        async (forceError = false) => {
            ...
            try {
                ...
                const message = isMockHttpError(loadError)
                    ? loadError.message
                    : 'Ofertele nu au putut fi incarcate.';

                setError(message);

                if (
                    isMockHttpError(loadError) &&
                    loadError.status === 500
                ) {
                    navigate(PATHS.SERVER_ERROR, {
                        state: { message },
                    });
                }
            } finally {
        ...
        <div className="app-shell min-h-screen">
            ...
            <main className="mx-auto w-full max-w-7xl px-4 py-10">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div>
                        ...
                        <p className="mt-2 max-w-2xl text-slate-600">
                            Vizualizeaza ofertele disponibile de la
                            maestrii cadrelor.
                        </p>
                    </div>
                ...
                <section className="mt-6">
                    {isLoading && (
                        <LoadingState title="Se incarca ofertele..." />
                    )}
                    ...
                    {!isLoading &&
                        ...
                        result.items.length === 0 && (
                            <EmptyState
                                title="Nu exista oferte pentru filtrele curente"
                                message="Schimba termenul de cautare sau categoria."
                            />
                        )}
                            ...
                            <>
                                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                    {result.items.map((offer) => (
                                        <article
                                            key={offer.id}
                                            className="soft-panel card-hover overflow-hidden"
                                        >
                                            <img
                                                src={
                                                    offer.coverImageUrl
                                            ...
                                            <div className="p-5">
                                                ...
                                                <div className="mt-5">
                                                    ...
                                                    'user' ? (
                                                        <Link
                                                            to={`${PATHS.USER_DASHBOARD}?offerId=${encodeURIComponent(
                                                                offer.id,
                                                            )}`}
